...
class EmojiSearchTab(wx.Panel):
    def __init__(self, parent):
        ...
        if self.user_settings.get_is_general_font_size_enabled():
            ...
            font_size = self.user_settings.get_search_tab_font_size()
    ...
    def OnDescClick(self, event):
        ...
        if self.user_profile.username != "guest":
            try:
                self.user_profile.used_emojis.update([EMOJI_UNICODE[desc]])
            except KeyError:
                self.user_profile.used_emojis.update([EMOJI_ALIAS_UNICODE[desc]])
    ...
    def OnTTS(self, event):
        os.system('say ' + self.emoji_symbol.emoji_desc + ' -r ' + str(self.user_settings.get_tts_speed()))
